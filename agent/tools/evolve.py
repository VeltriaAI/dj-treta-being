...
def evolve(goal: str, scope: str = "agent/", max_budget_usd: float = 0.50, run_tests: bool = True) -> str:
    ...
    try:
        ...
        try:
            ...
            db.execute(
                ...
                (goal, scope, "success" if "github.com" in pr_url else "failed", pr_url, branch, max_budget_usd, "manual")
            )
            db.commit()
            db.close()
        except Exception:
